...
{$REGION 'PESQUISAR'}
//pega o texto digitado,monta um where e executa um select filtrando tudo
...
begin
  ...
  if PosOrder > 0 then
  begin
    SQLSemOrder   := Copy(SelectOriginal, 1, PosOrder - 1); //começa na posição 1 e pega por exemplo 23 (total=24)
    OrderByClause := Copy(SelectOriginal, PosOrder, Length(SelectOriginal)); //fica Copy(SelectOriginal, 24, tamanho_total)
  end
  else
  begin
  ...
  // Monta mapa: Alias e CampoReal
  Mapa := TStringList.Create;
  Partes := TStringList.Create;
  try
    PosFrom := Pos('FROM', UpperCase(SelectOriginal)); //deixa td maiusculo
    SelectPart := Trim(Copy(SelectOriginal,
      Pos('SELECT', UpperCase(SelectOriginal)) + 6, //pega dps do select (6 letras)
      PosFrom - Pos('SELECT', UpperCase(SelectOriginal)) - 6)); //remove o select
    ...
    begin
      ...
      // Busca o CampoReal pelo Alias(nome) exato, percorrendo coluna por coluna
      NomeCampo := '';
      ...
      //cria pedaços do where automaticamente
      Condicao := '';
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        MessageDlg(
          'Todos os campos devem ser preenchidos!',mtInformation,[mbOK],0);
    ...
    begin
      ...
      begin
        MessageDlg(
          'Preencha todos os campos.',mtWarning,[mbOK],0);
...
begin
  ...
  ControlarBotoes(btnNovo,btnAlterar,btnCancelar,btnGravar,btnApagar,btnNavigator,True);
end;
{$ENDREGION}
...
begin
  ...
  Ini := TIniFile.Create(
    ExtractFilePath(Application.ExeName) + 'grid_padrao.ini');
...
begin
  ...
  try
    ...
    begin
      grdListagem.Columns[i].Width :=
        Ini.ReadInteger(Secao,'Width' + IntToStr(i),grdListagem.Columns[i].Width);
    end;
