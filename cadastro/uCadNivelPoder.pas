...
uses
  ...
  Vcl.ComCtrls, uEnum, cCadNivelPoder,uDTMConexao;


type
  TfrmCadNivelPoder = class(TfrmTelaHeranca)
    lbl15: TLabel;
    edtNivelId: TLabeledEdit;
    edtNivelPoder: TLabeledEdit;
...
var
  frmCadNivelPoder: TfrmCadNivelPoder;
...
procedure TfrmCadNivelPoder.btnApagarClick(Sender: TObject);
begin
  if not oNivel.Selecionar(QryListagem.FieldByName('id').AsInteger) then
    Exit;

  if MessageDlg(
     'Apagar o registro:'#13#13 +
     'Código: ' + QryListagem.FieldByName('id').AsString + #13 +
     'Nome: ' + QryListagem.FieldByName('nivel').AsString,
     mtConfirmation,
     [mbYes, mbNo],
     0) <> mrYes
  then
...
procedure TfrmCadNivelPoder.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  inherited;
  if Assigned(oNivel) then
    FreeAndNil(oNivel);
end;

procedure TfrmCadNivelPoder.FormCreate(Sender: TObject);
begin
  inherited;
  oNivel := TNivel.Create(dtmConexao.conexaoDB);
  IndiceAtual:= 'nivel';
end;
{$ENDREGION}
end.
